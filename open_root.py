...
local_file = uproot.open('test.root')
#pprint(local_file['events'].keys())

events = local_file['events']



for events in events.iterate(["ForwardTRDHits/ForwardTRDHits.momentum.z"], step_size=100, entry_start=0, entry_stop=2):
    for event in events:
        for particle in zip(event["ForwardTRDHits/ForwardTRDHits.momentum.z"]):
            print(particle)
